...
The header contains navigation, search, sorting, file operations, pane visibility toggles, terminal app launch, and path copy actions. The preview pane width can be changed by dragging its divider. Split mode starts the left and right file panes at equal width; while split mode remains visible, the divider between the file panes can be dragged to temporarily adjust the allocation. That temporary allocation is not persisted and resets to 50:50 the next time split mode is shown.

The horizontal pane layout (folder tree | file area | preview) is hosted by `MainPaneSplitView`, an `NSSplitView`-backed `NSViewRepresentable` that owns the window's `contentMinSize`. Pane visibility changes reallocate the current window width rather than directly resizing the window. The content minimum is derived from visible panes' hard minimum widths plus dividers, with the header-driven minimum kept as a floor. The folder tree keeps its stored width when possible, does not grow automatically when the window grows, and only shrinks toward its minimum after other eligible panes have reached their minimums. When the preview pane is visible, window-width changes first resize the preview pane; if the preview reaches its minimum width, further narrowing propagates into the file area. Window-resize-driven preview widths are transient and are not written to `previewWidth`. Each side pane has a `LayoutPane` enum case carrying its minimum / default widths and `UserDefaults` keys, and a unified `setVisible(_:_:)` / `setStoredWidth(_:_:)` read/write API on `TerminalFileManagerView` that every consumer (toolbar, menu, keyboard shortcut, drag handler) uses.
...
When the built-in terminal pane is visible, the window content minimum height includes both the main file area minimum height and the terminal pane minimum height, so resizing the window cannot collapse either area below its usable floor. Window-height changes first resize the terminal pane; if the terminal reaches its minimum height, further narrowing propagates into the main file area. Window-resize-driven terminal heights are transient and are not written to `terminalPaneHeight`.
...
File-list columns are resizable by dragging their headers. The left pane's column widths are persisted through `TerminalFileManager.fileColumnWidths` (with `TerminalFileManager.fileNameColumnWidth` retained as a legacy fallback for the name column). The right pane keeps its column widths only as transient in-session state, so right-pane adjustments are not restored on relaunch.
