...
| `rendered` | その拡張子をレンダリング表示で開始します。対応するテキスト系形式では、レンダリング/ソース表示の切り替えボタンは引き続き表示されます。 |
...
このテーブルは、主に Markdown、HTML、CSV、JSON、プレーンテキスト、ログなど、表示方法の選択に意味があるテキスト系形式に使います。画像、PDF、動画、その他 QuickLook 系の形式は通常ここで指定する必要はありません。これらは意味のある Raw text 表示を持たないため、`rendered` は通常のプレビュー表示とほぼ同じで、`text` を指定してもテキストとして読めるようにはなりません。

標準では、`toml`、`yaml`、`yml`、`ini`、`cfg`、`conf`、`log`、`txt`、`env` が直接 Raw text プレビューに送られます。Markdown（`md`、`markdown`、`mdown`、`mkd`）、CSV / TSV、JSON、HTML もテキスト系ですが、専用のレンダリング表示、またはレンダリング / ソース表示の切り替え対象として扱われます。
